...
var createChunksList = function createChunksList(valueX, valueY) {
...
var compareChunks = function compareChunks(valueX, valueY) {
  return createChunksList(valueX, valueY).reduce(compareChunkPair, 0);
};

var compareValues = function compareValues(valueA, valueB, caseSensitive) {
  var x = convertToString(caseSensitive, valueA);
  var y = convertToString(caseSensitive, valueB);
  var numberX = parseNumber(x);
  var numberY = parseNumber(y);

  var result = void 0;
  // first try and sort Hex codes or Dates
  if (numberX !== undefined && numberY !== undefined) {
    result = compareNumbers(numberX, numberY);
  } else {
    // natural sorting through split numeric strings and default strings
    result = compareChunks(x, y);
  }
  return result;
};
